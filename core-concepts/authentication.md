...
AuthHandler takes a [`RequestContext`](context.md#requestcontext) and returns a `Viewer` if that handler can successfully authenticate the Viewer. Otherwise, it returns `null`.
...
Note that `registerAuthHandler` should be called at the root of your application (presumably in the same file as your server such as `src/graphql/index.ts`). If your handler successfully authenticates a `Viewer`, that Viewer is returned as the `Viewer` in the `Context` returned by [`buildContext`](context.md#new-request).
