...
- `ActionOperation.AddEdge`: adds a new [edge](edges.md)
- `ActionOperation.RemoveEdge`: removes an [edge](edges.md)
- `ActionOperation.EdgeGroup`: creates a new action for an [edge group](edge-groups.md)
...
Overrides the default [input](../actions/input.md) name created.
...
sometimes, we need fields that are not in the schema that need to be generated in the [action input](../actions/input.md) or in the [input to the graphql mutation](https://graphql.org/learn/schema/#input-types).
...
- `nullable`: if true, item is nullable e.g. `string | null`. if a list, it gets slightly more complicated
	- if a list and `nullable` is `true`, the list itself is nullable but not its contents. The GraphQL type here would be `[String!]`
		...
		- if a list and `nullable` is `contentsAndList`, both the list and its contents are nullable. The GraphQL type here would be `[String]` [Check out](https://graphql.org/learn/schema/#lists-and-non-null) for more about lists and null in GraphQL.
...
This allows us to create the event and its associated address in the same request using [triggers](../actions/triggers.md).

See [Action Only Fields](../actions/action-only-fields.md) for more.
...
Inverse of [requiredField](#requiredfield) above. Want to make a field which would be required optional. Usually means that a default value is set in a [trigger](../actions/triggers.md)
