...
Edge Groups supports 1 [action](actions.md). It's configured as follows:
...
- `actionOnlyFields`: specify custom fields that are generated on this action (and in GraphQL) that don't map to something in the schema or db. More on [action only fields](../actions/action-only-fields.md).
