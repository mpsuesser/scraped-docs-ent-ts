...
For every [edge](../ent-schema/edges.md) configured, we generate an EntQuery for it.
...
Since edges are sorted by [time in descending order](../ent-schema/edges.md#database), to reverse the order, you can query as follows:
...
For an [indexed foreign key](../ent-schema/fields.md#foreignkey) or an [index](../ent-schema/fields.md#index) e.g.
...
For the [edge based query](#edge-based), the edge class can be customized to add extra logic, for example based on what's in the `data` or `time` [fields](../ent-schema/edges.md#database).
...
This can be done by adding a [Privacy Policy](privacy-policy.md) to the query class.
...
This ent is what's passed to each [PrivacyPolicyRule](privacy-policy.md) to evaluate if the query is visible.

## ents

Note that when querying ents at the end of a query, we do privacy aware loading and so we only return nodes at the end of the edge that are visible based on the ent's [privacy policy](ent.md#privacy-policy).
