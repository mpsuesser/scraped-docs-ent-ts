...
If provided, used as the name of the database column. Otherwise, a snake\_case version of the name is used. e.g. `first_name` for `FirstName` or `firstName` or `firstname`.
...
```
- we have a 1-many [Edge](edges.md) from `User` to `Event` for events the User has created.
...
- [Builder](../actions/builder.md)
- [Input](../actions/input.md)

This can be used to compute a value at runtime. For example, to default to the [Viewer](../core-concepts/viewer.md) in the todo app above, you can do:
