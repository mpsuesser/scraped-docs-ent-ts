...
- a `TypeScript` file in snake\_case format e.g. `user_schema.ts`, `event_schema.ts`, `pickup_location_schema.ts` etc.
...
If you have structs, enums, or edge behaviors that need to be shared across multiple schemas, define them once via a [global schema](global-schema.md) and reference them via `globalType` inside each schema.
...
For a deep dive into fields, you can learn more [here](fields.md).
...
Configures the types of writes that can be performed on this object. For a deep dive into actions, please [visit](../actions/action.md).

### enumTable

Visit [Enums](enums.md) to learn more.

### dbRows

Visit [Enums](enums.md) to learn more. Can also be used without enums.

### constraints

database constraints added to the table. [Visit](constraints.md) to learn more.

### indices

database indices added to the table. [Visit](indices.md) to learn more.
...
`EntSchema` is a class that uses a [Pattern](patterns.md) and adds 3 fields to the object:
...
`EntSchemaWithTZ` is a schema that uses a [Pattern](patterns.md) and adds 3 fields to the object:
